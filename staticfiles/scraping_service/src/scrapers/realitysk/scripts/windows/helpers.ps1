...
<#
.SYNOPSIS
   Checks if given service name is running.
.DESCRIPTION
   This function detects whether a service is running.
#>
function Running-ServiceAction {
    param (
        [string]$ServiceName
    )
...
<#
.SYNOPSIS
   Checks if master scraping service is running.
.DESCRIPTION
   This function detects whether a master scraping service is running.
#>
function MasterServiceIsRunning-ServiceAction {
    try {
        if (-not (Running-ServiceAction $masterServiceName)) {
            throw "Master service is not running. Aborting."
        }
    } catch {
        Write-Host "[ERROR]: $_"
        exit 1
...
function Install-ServiceAction {
    try {
        MasterServiceIsRunning-ServiceAction
